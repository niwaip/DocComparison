...
resultEl.addEventListener("click", async (e) => {
  ...
  const aiApiKey = String(aiApiKeyEl?.value || "").trim();
  ...
  try {
    ...
    const payload = blockId
      ? { blockId, focusText: focusText || null, aiApiKey: aiApiKey || null }
      : { rowId, focusText: focusText || null, aiApiKey: aiApiKey || null };
...
formEl.addEventListener("submit", async (e) => {
  ...
  if (aiApiKeyEl) {
    const key = String(aiApiKeyEl.value || "").trim();
    if (key) fd.set("aiApiKey", key);
  }
