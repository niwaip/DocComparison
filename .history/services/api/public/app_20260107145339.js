...
resultEl.addEventListener("click", async (e) => {
  ...
  const rawSel = window.getSelection ? String(window.getSelection().toString() || "") : "";
  ...
  setStatus("AI 差异解析中…");
  try {
    const res = await fetch(`/api/compare/${encodeURIComponent(currentCompareId)}/ai/snippet`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ rowId, focusText: focusText || null })
    });
    if (!res.ok) throw new Error(await res.text());
