...
app.post("/api/compare", upload.fields([{ name: "leftFile", maxCount: 1 }, { name: "rightFile", maxCount: 1 }]), async (req, res) => {
  try {
    ...
    const [leftHtml, rightHtml] = await Promise.all([docxBufferToSafeHtml(left.buffer), docxBufferToSafeHtml(right.buffer)]);
    const leftBlocks = buildBlocksFromHtml(leftHtml);
    const rightBlocks = buildBlocksFromHtml(rightHtml);
    ...
    res.json({
      ...
      ai: { mode: compareJson.ai.mode, status: compareJson.ai.status, jobId: compareJson.ai.jobId, pollUrl: compareJson.ai.jobId ? `/api/ai/jobs/${compareJson.ai.jobId}?compareId=${compareId}` : null }
    });
  } catch (e: any) {
