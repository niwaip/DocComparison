...
app.post("/api/compare", upload.fields([{ name: "leftFile", maxCount: 1 }, { name: "rightFile", maxCount: 1 }]), async (req, res) => {
  try {
    ...
    const blockMaxChars = parsePositiveInt(String(req.body?.blockMaxChars ?? "").trim());
    const pdfDefaultMax = parsePositiveInt(env("PDF_BLOCK_MAX_CHARS", "280"));
    const maxBlockChars =
      blockMaxChars ??
      ((String(left.mimetype ?? "").toLowerCase().includes("pdf") || String(right.mimetype ?? "").toLowerCase().includes("pdf")) ? pdfDefaultMax : null);
    ...
    const leftBlocks = buildBlocksFromHtml(leftHtml, { maxBlockChars: maxBlockChars === null ? undefined : maxBlockChars });
    const rightBlocks = buildBlocksFromHtml(rightHtml, { maxBlockChars: maxBlockChars === null ? undefined : maxBlockChars });
    const rows = alignBlocks(leftBlocks, rightBlocks);
    const sectionNumberChangedRows = rows.filter((r) => r.meta?.sectionNumberChanged).length;
    ...
    for (const row of rows) {
      ...
      const d = inlineDiffHtml(lb.text, rb.text);
      row.diff = { diffHtmlFragment: `<p>${d.html}</p>` };
      modifiedCount++;
    }
