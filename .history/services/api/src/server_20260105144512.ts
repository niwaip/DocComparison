...
app.post("/api/compare", upload.fields([{ name: "leftFile", maxCount: 1 }, { name: "rightFile", maxCount: 1 }]), async (req, res) => {
  try {
    ...
    const leftBlocks = buildBlocksFromHtml(leftHtml, { maxBlockChars: maxBlockChars ?? undefined });
    const rightBlocks = buildBlocksFromHtml(rightHtml, { maxBlockChars: maxBlockChars ?? undefined });
