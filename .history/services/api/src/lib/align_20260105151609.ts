...
export function alignBlocks(left: Block[], right: Block[]): AlignmentRow[] {
  ...
  return coalesceModified(rows, left, right);
}

function coalesceModified(rows: AlignmentRow[], left: Block[], right: Block[]): AlignmentRow[] {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (const d of delRun) {
      ...
      if (bestIdx >= 0 && bestScore >= 0.35) {
        ...
        const onlySectionChange = stripSectionNoise(lb.text) === stripSectionNoise(rb?.text ?? "") && sectionNumberChanged;
        out.push({
          rowId: d.rowId,
          kind: onlySectionChange ? "matched" : "modified",
          leftBlockId: d.leftBlockId,
          rightBlockId: insRun[bestIdx].rightBlockId,
          meta: sectionNumberChanged
