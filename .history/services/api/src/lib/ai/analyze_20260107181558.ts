...
export async function analyzeRisks(params: {
  ...
  if (!overall.summary.trim()) {
    overall.summary = `共检测到 ${withSection.length} 处变更，覆盖 ${sectionLabels.length} 个一级章节。建议优先复核：责任、付款、终止、争议解决、数据合规等高风险条款。`;
  }
  if (overall.suggestions.length === 0) {
    overall.suggestions = ["先通读变更汇总，再逐章核对关键条款与定义", "对涉及金额/期限/责任上限/争议管辖的变更做重点标注与复核"];
  }
