...
function coalesceModified(rows: AlignmentRow[], left: Block[], right: Block[], options: { ignoreSectionNumber: boolean }): AlignmentRow[] {
  ...
  if (!options.ignoreSectionNumber) return normalized;

  const canonicalAfterStrip = (b: Block): string => {
    return stripSectionNoise(b.text)
      .toLowerCase()
      .replace(/[：∶︰﹕]/g, ":")
      .replace(/\s+/g, "")
      .trim();
  ...
  return normalized.map((r) => {
    if (r.kind !== "matched") return r;
    if (!r.leftBlockId || !r.rightBlockId) return r;
    const lb = leftMap.get(r.leftBlockId);
    const rb = rightMap.get(r.rightBlockId);
    if (!lb || !rb) return r;
    const beforeLabel = getLeadingSectionLabel(lb.text);
    const afterLabel = getLeadingSectionLabel(rb.text);
    if (!beforeLabel || !afterLabel || beforeLabel === afterLabel) return r;
    if (canonicalAfterStrip(lb) !== canonicalAfterStrip(rb)) return r;
    return {
      ...r,
...
function coalesceRunInOrder(
  ...
  const canonicalAfterStrip = (b: Block | undefined): string => {
  ...
  const isGoodMatch = (a: Block | undefined, b: Block | undefined): boolean => {
    ...
    if (options.ignoreSectionNumber) {
      const la = a ? getLeadingSectionLabel(a.text) : null;
      const lb = b ? getLeadingSectionLabel(b.text) : null;
      if (la && lb && la === lb) {
    ...
    const la = a ? getLeadingSectionLabel(a.text) : null;
    const lb = b ? getLeadingSectionLabel(b.text) : null;
    if (options.ignoreSectionNumber && la && lb && la !== lb) threshold = Math.max(threshold, 0.78);
    return s >= threshold;
  };
  ...
  const bestInRight = (lb: Block | undefined, startJ: number): { k: number; score: number } => {
    let bestK = -1;
    let bestScore = 0;
  ...
  const bestInLeft = (rb: Block | undefined, startI: number): { k: number; score: number } => {
    let bestK = -1;
    let bestScore = 0;
  ...
  while (i < delRun.length || j < insRun.length) {
    ...
    if (bestRight.k > 0 && bestRight.score >= currentScore + 0.08) {
      for (let t = 0; t < bestRight.k; t++) out.push(insRun[j + t]!);
      j += bestRight.k;
      continue;
    }
    ...
    if (bestLeft.k > 0) {
      for (let t = 0; t < bestLeft.k; t++) out.push(delRun[i + t]!);
      i += bestLeft.k;
      continue;
    }
