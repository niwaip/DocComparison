...
exports.getLeadingSectionLabel = getLeadingSectionLabel;
exports.stripSectionNoise = stripSectionNoise;
function normalizeText(input) {
    ...
    while (cjkSpace.test(out))
        out = out.replace(cjkSpace, "$1$2");
    return out;
}
...
function bigrams(s) {
    ...
    for (let i = 0; i < x.length - 1; i++)
        out.push(x.slice(i, i + 2));
    return out.length ? out : [x];
}
function getLeadingSectionLabel(text) {
    const norm = normalizeText(text);
    const firstLine = norm.split("\n").map((x) => x.trim()).find((x) => x.length > 0) ?? "";
    if (!firstLine)
        return null;
    const mNum = /^(\d+(?:\s*\.\s*\d+)*)(?=(?:\s|[.。:：、\-—\)])|$)/.exec(firstLine);
    if (mNum)
        return mNum[1].replace(/\s+/g, "");
...
function stripSectionNoise(text) {
    ...
    const lines = norm.split("\n").map((line) => {
        ...
        if (!s)
            return "";
        s = s
            .replace(/(?:(?:\.\s*){3,}|\.{3,}|(?:…\s*){2,}|…{2,}|(?:·\s*){3,}|·{3,}|-{3,}|_{3,})\s*\d+\s*$/g, "")
            .replace(/\s{3,}\d+\s*$/g, "")
            .replace(/(?:\.\s*){6,}\s*$/g, "")
            .replace(/\.{6,}\s*$/g, "")
            .trim();
        ...
        if (mNum) {
            ...
            if (mCn)
                s = s.slice(mCn[0].length).replace(/^[\s.。:：、\-—)\]]+/g, "").trimStart();
        }
