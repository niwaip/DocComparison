...
exports.renderCompareHtml = renderCompareHtml;
function renderCompareHtml(params) {
    ...
    const body = rows
        ...
        if (row.kind === "inserted" && right)
            rightFinal = wrapCellHtml("right", right.blockId, right.htmlFragment, true);
        if (row.kind === "deleted" && left)
            leftFinal = wrapCellHtml("left", left.blockId, left.htmlFragment, true);
...
function wrapCellHtml(side, blockId, fragment, highlight = false) {
