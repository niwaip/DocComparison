...
function inlineDiffHtml(beforeText, afterText, options) {
    ...
    const shouldAlignLines = (aLines, bLines, ignoreSectionNumber) => {
        const key = (line) => {
            const x = ignoreSectionNumber ? (0, text_1.stripSectionNoise)(line) : line;
            return x.toLowerCase().replace(/\s+/g, " ").trim();
        };
        const scoreLine = (line) => {
            const s = (line ?? "").trim();
            if (!s)
                return { len: 0, anchor: false };
            const len = s.length;
        ...
        const analyze = (lines) => {
            ...
            if (n === 0)
                return { n: 0, shortRatio: 0, anchorRatio: 0, avgLen: 0 };
    ...
    const aLines0 = before.split("\n");
    const bLines0 = after.split("\n");
    const hasShortAnchoredLines = (lines) => {
        const bulletRe = /^\s*(?:[a-z]\)\s+|\d+\)\s+|[-*•]\s+|[（(]?[一二三四五六七八九十]+[）)]\s*|[一二三四五六七八九十]+[、.]\s*)/i;
        const anchorN = lines.filter((l) => bulletRe.test((l ?? "").trim())).length;
        return lines.length >= 2 && anchorN >= 2;
    };
    if (alignEnabled) {
        const aLines = aLines0;
        const bLines = bLines0;
    ...
    if (!preserveHardLineBreaks) {
        preserveHardLineBreaks = hasShortAnchoredLines(aLines0) || hasShortAnchoredLines(bLines0);
    }
    ...
    const renderText = (data) => {
        const escaped = (0, text_1.escapeHtml)(data);
        if (preserveHardLineBreaks)
            return escaped.replace(/\n+/g, "<br/>");
        return escaped.replace(/\n+/g, " ").replace(/ {2,}/g, " ");
    };
    const leftHtml = normalizeRenderedHtml(diffs
        .map(([op, data]) => {
        ...
        if (op === diff_match_patch_1.default.DIFF_DELETE) {
            const id = `d_${delN++}`;
            delIds.push(id);
    ...
    insN = 1;
    delN = 1;
    const rightHtml = normalizeRenderedHtml(diffs
        .map(([op, data]) => {
        const escaped = renderText(data);
        if (op === diff_match_patch_1.default.DIFF_INSERT) {
            const id = `i_${insN++}`;
...
function alignLinesWithLookahead(leftLines, rightLines, window, ignoreSectionNumber) {
    const key = (line) => {
        const x = ignoreSectionNumber ? (0, text_1.stripSectionNoise)(line) : line;
        return x.toLowerCase().replace(/\s+/g, " ").trim();
    };
    ...
    const parseLabel = (line) => {
        const label = (0, text_1.getLeadingSectionLabel)(line);
        if (!label)
            return null;
        if (!/^\d+(?:\.\d+)*$/.test(label))
            return null;
    ...
    while (i < leftLines.length || j < rightLines.length) {
        ...
        if (i + 1 < leftLines.length && canSoftJoin(a, leftLines[i + 1] ?? "")) {
            ...
            if ((0, text_1.diceCoefficient)(key(mergedLeft), key(b)) >= 0.92) {
        ...
        if (j + 1 < rightLines.length && canSoftJoin(b, rightLines[j + 1] ?? "")) {
            ...
            if ((0, text_1.diceCoefficient)(key(a), key(mergedRight)) >= 0.92) {
                outL.push(a);
        ...
        const sim = (0, text_1.diceCoefficient)(key(a), key(b));
