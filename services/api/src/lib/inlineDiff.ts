...
export function inlineDiffHtml(
  ...
  const hasShortAnchoredLines = (lines: string[]): boolean => {
    const bulletRe =
      /^\s*(?:[a-z]\)\s+|\d+\)\s+|[-*•]\s+|[（(]?[一二三四五六七八九十]+[）)]\s*|[一二三四五六七八九十]+[、.]\s*)/i;
  ...
  const normalizeRenderedHtml = (html: string): string => {
    let out = html ?? "";
  ...
  const leftHtml = normalizeRenderedHtml(
    diffs
  ...
  const rightHtml = normalizeRenderedHtml(
    diffs
      .map(([op, data]: [number, string]) => {
        const escaped = renderText(data);
        if (op === DiffMatchPatch.DIFF_INSERT) {
