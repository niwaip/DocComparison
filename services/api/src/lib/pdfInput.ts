...
function looksLikeSectionHeading(line: string): boolean {
  const s = line.trim();
  if (!s) return false;
  if (getLeadingSectionLabel(s)) return true;
...
function reorderTermBlocks(paras: string[]): string[] {
  ...
  const isTermLabelPara = (p: string): { major: number; segs: number[] } | null => {
    ...
    if (new RegExp(`^${escapeRegExp(label)}(?:\\s|[.。:：、\\-—)）])`).test(s)) return { major: segs[0] ?? -1, segs };
    return null;
  };

  const sortGroupsIfNeeded = (groups: Array<{ major: number; segs: number[]; paras: string[] }>): string[] => {
    if (groups.length < 2) return groups.flatMap((g) => g.paras);
    ...
    if (groups.length < 5) {
      ...
      if (!allSameDepth || !allWithinFirst) return groups.flatMap((g) => g.paras);

      const second = groups.map((g) => g.segs[1]).filter((n): n is number => Number.isFinite(n));
      if (second.length !== groups.length) return groups.flatMap((g) => g.paras);
      const spread = Math.max(...second) - Math.min(...second);
      if (spread > 12) return groups.flatMap((g) => g.paras);
    }
