...
describe('api', () => {
      ...
      const fetchMock = vi.fn(async () =>
        makeRes({
          ...
          json: async () => ({
            ...
            templateVersion: 'v1',
            items: [],
            summary: { counts: { pass: 0, fail: 0, warn: 0, manual: 0, error: 0, skipped: 0 } }
          })
        })
      )
  ...
  it('checkRun throws on invalid response shape', async () => {
    try {
      vi.stubGlobal(
        'fetch',
        vi.fn(async () =>
      ...
      await expect(api.checkRun('tid', [], false)).rejects.toThrow(/Invalid response: \/api\/check\/run/)
    } finally {
      vi.unstubAllGlobals()
    }
  })
